...
Question 1 (easy):
What's the fundamental difference between {{ }} and {% %} in Jinja — and which one would you use to call a macro that returns a calculated value to be inserted into your SQL?

---

Question 2 (easy-medium):
You want your fct_orders model to be materialized as table in production but view in your personal dev environment (to save warehouse compute while iterating). Which built-in Jinja variable would you use inside a {% if %} block to detect which environment you're running in, and roughly how would you write it?

---

Question 3 (medium, scenario):
You've written a macro {% macro get_payment_methods() %} that returns a Jinja list (not a SQL value) — say, ['credit_card', 'paypal', 'bank_transfer'] — read from a var() so it's configurable per environment. You want to use this list inside a {% for %} loop to generate one sum(case when ...) column per payment method (similar to the pivot example from the lesson). What return-mechanism issue do you need to be careful about here, compared to a macro like cents_to_dollars() that returns a plain SQL expression string?

---

Question 4 (medium-hard, scenario):
Your team's dbt-utils dependency is pinned in packages.yml as:
...
A new hire on your team runs dbt deps for the first time in 6 months and suddenly a bunch of models fail to compile — errors point to a dbt_utils macro that used to work fine. Nobody changed any project code. What's the most likely root cause here, and how would you both diagnose it and prevent this from happening again?
